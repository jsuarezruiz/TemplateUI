...
namespace TemplateUI.DataVisualization
{
    public class ValueAxis : TemplatedView
    {
        ...
        public static readonly BindableProperty AxisColorProperty =
            BindableProperty.Create(nameof(AxisColor), typeof(Color), typeof(ValueAxis), Colors.Black);
        ...
        void UpdateLocation()
        {
            ...
            for (int i = 0; i < _valueBoxes.Count; i++)
            {
                ...
                AbsoluteLayout.SetLayoutBounds(_valueBoxes[i], new Rectangle(_valueBoxes[i].X + _valueTicks[i].Width, y - _valueBoxes[i].Height / 2, _valueBoxes[i].Width, _valueBoxes[i].Height));
                AbsoluteLayout.SetLayoutBounds(_valueTicks[i], new Rectangle(_valueTicks[i].X + _valueTicks[i].Width, y, _valueTicks[i].Width, _valueTicks[i].Height));
            }
        }
    }
}