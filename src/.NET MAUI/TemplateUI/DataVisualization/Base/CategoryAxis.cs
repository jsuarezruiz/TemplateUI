...
namespace TemplateUI.DataVisualization
{
    public class CategoryAxis : TemplatedView
    {
        ...
        public static readonly BindableProperty AxisColorProperty =
           BindableProperty.Create(nameof(AxisColor), typeof(Color), typeof(CategoryAxis), Colors.Black);
        ...
        void UpdateLocation()
        {
            for (int i = 0; i < _valueLabels.Count; i++)
            {
                AbsoluteLayout.SetLayoutBounds(_valueLabels[i],
                    new Rectangle(_locations[i] - _valueLabels[i].Width / 2, _valueLabels[i].Y - _valueTicks[i].Height, _valueLabels[i].Width, _valueLabels[i].Height));
                AbsoluteLayout.SetLayoutBounds(_valueTicks[i],
                    new Rectangle(_locations[i], _valueTicks[i].Y - _valueTicks[i].Height, _valueTicks[i].Width, _valueTicks[i].Height));
            }
        }
    }
}